...
describe("Hash Valley tests", function () {
  ...
  const deploy = async () => {
    ...
    vineyard = await Vineyard.deploy(config.vine_base_uri, config.vine_img_uri, storage.address);
    ...
    bottle = await WineBottle.deploy(config.bottle_base_uri, config.bottle_img_uri, storage.address);
  ...
  describe("Setup and minting", function () {
    ...
    it("first 100 are free, 0.05 eth after that", async () => {
      for (let i = 0; i < 100; i++) {
        const tx = await vineyard.connect(accounts[1]).newVineyards([4,2,0,4])
        expect(tx).to.emit(vineyard, "Transfer").withArgs("0x0000000000000000000000000000000000000000", accounts[1].address, i)
      }
      await expect(vineyard.connect(accounts[1]).newVineyards([4,2,0,4], { value: ethers.utils.parseEther("0.04") }))
        .to.be.revertedWith('Value below price');

      const tx = await vineyard.connect(accounts[1]).newVineyards([4,2,0,4], { value: ethers.utils.parseEther("0.05") })
      expect(tx).to.emit(vineyard, "Transfer").withArgs("0x0000000000000000000000000000000000000000", accounts[1].address, 100)
    })

    it("Correct number of params", async () => {
      await expect(vineyard.connect(accounts[1]).newVineyards([1, 2, 3, 4, 5]))
        .to.be.revertedWith('Incorrect number of params');

      await expect(vineyard.connect(accounts[1]).newVineyards([1, 2, 3]))
        .to.be.revertedWith('Incorrect number of params');
    })

    it("Third attribute must be 0 or 1", async () => {
      await expect(vineyard.connect(accounts[1]).newVineyards([4, 2, 3, 4]))
    ...
    it("Only owner can withdraw", async () => {
      ...
      await expect(vineyard.connect(accounts[1]).withdrawAll())
    ...
    it("can't plant before game start", async () => {
      await vineyard.connect(accounts[1]).newVineyards([12, 13, 0, 4])
      await expect(vineyard.connect(accounts[1]).plant(0))
        .to.be.revertedWith('Not planting time');
    })

    it("can't harvest before game start", async () => {
      await vineyard.connect(accounts[1]).newVineyards([12, 13, 0, 4])
      await expect(vineyard.connect(accounts[1]).harvest(0))
  ...
  describe("Game flow", function () {
    ...
    it("watering", async () => {
      ...
      await expect(vineyard.connect(accounts[1]).water(0))
    ...
    it("can't harvest early", async () => {
      ...
      await ethers.provider.send("evm_increaseTime", [time]);
      expect(await vineyard.connect(accounts[1]).harvest(0)).to.emit(vineyard, "Harvested").withArgs(0, 1, 0)
      console.log(await bottle.tokenURI(0))

      await ethers.provider.send("evm_increaseTime", [time]);
      await expect(vineyard.connect(accounts[1]).harvest(0))
        .to.be.revertedWith('Vineyard already harvested');
    })
    ...
    it("second season", async () => {
      ...
      // can harvest on time
      await ethers.provider.send("evm_increaseTime", [time]);
      expect(await vineyard.connect(accounts[1]).harvest(0)).to.emit(vineyard, "Harvested").withArgs(0, 2, 0)
    })
    ...
    it("water multiple", async () => {
      ...
      await expect(vineyard.connect(accounts[1]).waterMultiple([0, 1, 2]))
        .to.be.revertedWith('Vineyard can\'t be watered');
    })
    ...
    it("harvest multiple", async () => {
      ...
      // can't harvest early
      await expect(vineyard.connect(accounts[1]).harvestMultiple([0, 1, 2]))
        .to.be.revertedWith('Not harvest time');
  ...
  describe("Cellar", function () {
    ...
    it("stake bottle", async () => {
      ...
      await expect(bottle.ownerOf(0))
        .to.be.revertedWith('ERC721: owner query for nonexistent token');
      await expect(bottle.ownerOf(2))
        .to.be.revertedWith('ERC721: owner query for nonexistent token');
      ...
      await expect(bottle.rejuvenate(0))
        .to.be.revertedWith('cannot rejuve');
    })
  })
});
